...
- __Run watchers__<br /> If you want to keep your database up-to-date with the blockchain, you can run event watchers. Currently we watch two contracts: `npm run watch-marketplace` and `npm run watch-land`. Keep in mind that the address you use for each contract will determine the network. For more information in event watching, check [here](https://github.com/decentraland/marketplace/tree/master/scripts/monitor).
